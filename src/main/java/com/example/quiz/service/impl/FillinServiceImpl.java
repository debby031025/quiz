...
public class FillinServiceImpl implements FillinService {
	...
	public BasicRes fillin(FillinReq req) {
		...
		try {
			...
			for (Question item : quList) { // 1(必) 2(必) 3(選)
				...
				for (Fillin fillin : fillinList) { // 1,2,3,4 or 1,2,3,3 or 1,3
					// id 不一致，跳過
				...
				// 正常的情況是: 問題是必填，然後又有回答，每跑完一題後，qIds 就會包含該必填問題的 id
	...
	public StatisticsRes statistics(FeedbackReq req) {
		...
		// 計算所有回答之題號,選項,次數
		//  qId(題號)    選項    次數
		Map<Integer,Map<String,Integer>> countMap = new HashMap<>(); // map特性會後蓋前 所以放response上 for迴圈之外
			...
			try {
				...
				for(Fillin fillin : fillinList) {
					...
					for(String option : optionArray) { // optionArray:綠茶,綠茶拿鐵,紅茶拿鐵,梅子綠茶
						...
						// 所以需要在每個選項""前後""再加上分號，會用分號是因為答案中的串接使用分號
						// 後續要找出現次數時就會是用 ;綠茶; 來找
