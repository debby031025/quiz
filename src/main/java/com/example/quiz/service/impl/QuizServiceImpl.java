...
public class QuizServiceImpl implements QuizService {
	...
	public BasicRes createOrUpdate(CreateOrUpdateReq req) {
		...
		// 因為 Quiz 中的 questions 的資料格式是 String，所以要將 req 的List<Question> 轉成 String
		// 透過 ObjectMapper 可以把物件(類別) 轉成 JSON格式的字串
		ObjectMapper mapper = new ObjectMapper();
		try {
			String questionStr = mapper.writeValueAsString(req.getQuestionList());

			// 若 req 中的 id>0，表示更新已存在的資料;若 req 中的 id=0，則表示新增資料
			if (req.getId() > 0) {
				// 以下兩種方式擇一
				// 使用方法一的話，透過 findById 若有資料，就會回傳一整筆的資料(可能資料量會較大)
				// 使用方法二的話，因為是透過 existsById 來判斷資料是否存在，所以回傳的資料永遠都只會是一個bit(0或1)
				// 方法一: 透過 findById()
...
//				quiz.setPublished(req.isPublished());
				...
				// 方法二: 透過 existsById:回傳一個 bit 的值
				// 這邊要判斷從 req (test或postman) 帶進來的 id 是否真的有存在於資料庫中
...
//			quizDao.save(quiz);
			// 因為變數 quiz 只使用一次，因此可以使用匿名類別的方式撰寫(不需要變數接)
			// new Quiz()中帶入 req.getId() 是PK，在呼叫 save 時，會先去檢查 PK 是否有存在於資料庫中
			// 若存在 --> 更新; 若不存在 --> 新增
			// req 中沒有該欄位時，預設是 0，因為 id 的資料型態是 int(預設值是 0)
...
//		}
	...
	private BasicRes checkParams(CreateOrUpdateReq req) {
		// 檢查問卷參數
		// StringUtils.hasText(字串):會檢查字串是否為 null,空字串,全空白字串，若是符合3種其中一項，會回false
		...
		// 所以後續檢查結束時間時，只要確定結束時間大於等於開始時間即可，因為只要結束時間是大於等於開始時間，就一定會是大於等於當前時間
		...
		for (Question item : req.getQuestionList()) {
			...
			// 以下條件檢查:當option_type 是單選或多選時，且 options 是空字串，返回錯誤
